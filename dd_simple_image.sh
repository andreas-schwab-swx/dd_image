...
# zstd
start_time=$(date +%s)
dd if="$DISK_DEVICE" bs=32M status=progress | mbuffer -m 1G -q | zstd -T3 -3 > "$MOUNT_DIR/$BACKUP_FILENAME"
...
# pigz
start_time=$(date +%s)
dd if="$DISK_DEVICE" bs=32M status=progress | mbuffer -m 1G -q | pigz -p3 > "$MOUNT_DIR/$BACKUP_FILENAME_PIGZ"
duration=$(($(date +%s) - start_time))
...
# bzip2
start_time=$(date +%s)
dd if="$DISK_DEVICE" bs=32M status=progress | mbuffer -m 1G -q | bzip2 -3 > "$MOUNT_DIR/$BACKUP_FILENAME_BZ2"
duration=$(($(date +%s) - start_time))
...
# gzip
start_time=$(date +%s)
dd if="$DISK_DEVICE" bs=32M status=progress | mbuffer -m 1G -q | gzip -3 > "$MOUNT_DIR/$BACKUP_FILENAME_GZ"
duration=$(($(date +%s) - start_time))
...
# lz4
start_time=$(date +%s)
dd if="$DISK_DEVICE" bs=32M status=progress | mbuffer -m 1G -q | lz4 -3 > "$MOUNT_DIR/$BACKUP_FILENAME_LZ4"
duration=$(($(date +%s) - start_time))
...
# xz
start_time=$(date +%s)
dd if="$DISK_DEVICE" bs=32M status=progress | mbuffer -m 1G -q | xz -T3 -3 > "$MOUNT_DIR/$BACKUP_FILENAME_XZ"
duration=$(($(date +%s) - start_time))
